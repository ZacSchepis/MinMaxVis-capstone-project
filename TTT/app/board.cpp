...
void Board::update_cell(int row, int col) {
    ...
    if (item && (p_type != error_state)) {
        ...
        button->setIconSize(QSize(cell_size-2, cell_size-2)); // Make sure the icon fits the button
        button->setStyleSheet("font-size: 24px; font-weight: bold;"); // Styling the text
    }
}
